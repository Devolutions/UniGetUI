...
internal static class TuiPages
{
    ...
    private static void AppendSourcesSection(StackPanel panel, IReadOnlyList<IPackageManager> managers)
    {
        ...
        foreach (var manager in managers)
        {
            ...
            try { sources = manager.SourcesHelper.GetSources(); }
            catch { continue; }
    ...
    {
        try { return source.Url?.ToString() ?? string.Empty; }
    ...
    private static Control ManagerRow(string name, string version, string status, bool isHeader, bool isReady = false)
    {
        ...
        {
            statusBlock.Foreground = isReady ? DevolutionsPalette.SuccessTextBrush : new SolidColorBrush(Color.Parse("#C0C0C0"));
        }
    ...
    {
        try { return m.IsReady(); } catch { return false; }
    }

    private static bool SafeIsEnabled(IPackageManager m)
    {
        try { return m.IsEnabled(); } catch { return false; }
    }
}
